<div>
    @php $title = 'Master Bidang'; @endphp
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">Master Bidang</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">Kelola bidang/divisi kegiatan DLH</p>
        </div>
        <button wire:click="openCreate" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium transition-colors">
            + Tambah Bidang
        </button>
    </div>
    ...
    <div class="bg-white dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mb-4 p-4 transition-colors duration-200">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari bidang..." class="w-full sm:w-72 rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 text-sm focus:border-emerald-500 focus:ring-emerald-500">
    </div>
    ...
    <div class="bg-white dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden transition-colors duration-200">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left">Total Kegiatan</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                @forelse($bidangs as $bidang)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="px-6 py-3 font-medium text-slate-800 dark:text-slate-100">{{ $bidang->nama }}</td>
                        <td class="px-6 py-3 text-slate-500 dark:text-slate-400">{{ $bidang->slug }}</td>
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300">{{ Str::limit($bidang->deskripsi, 50) ?: '-' }}</td>
                        <td class="px-6 py-3 text-slate-600 dark:text-slate-300">{{ $bidang->kegiatans_count }}</td>
                        <td class="px-6 py-3 text-right space-x-2">
                            <button wire:click="openEdit({{ $bidang->id }})" class="text-emerald-600 hover:underline">Edit</button>
                            <button wire:click="delete({{ $bidang->id }})" wire:confirm="Hapus bidang ini? Kegiatan yang terkait akan kehilangan referensi bidang." class="text-red-600 hover:underline">Hapus</button>
                        </td>
                    </tr>
                @empty
        ...
        <div class="px-6 py-3 border-t">{{ $bidangs->links() }}</div>
    </div>
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-slate-850 rounded-xl shadow-xl w-full max-w-md p-6" @click.outside="$wire.closeModal()">
                <h3 class="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-100">{{ $editingId ? 'Edit' : 'Tambah' }} Bidang</h3>
                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Bidang</label>
